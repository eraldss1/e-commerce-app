...
public class ShoppingCartFragment extends Fragment implements ICheckBoxChangedListener {
    ...
    public void updateTotal(double total, double charge) {
        ...
        txtSubTotal.setText(String.format("Rp %s", formatter.format(total)));
        txtDeliveryCharge.setText(String.format("Rp %s", formatter.format(charge)));
        txtTotal.setText(String.format("Rp %s", formatter.format(total + charge)));
    }
}